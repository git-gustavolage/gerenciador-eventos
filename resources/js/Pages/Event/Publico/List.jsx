...
function CustomSelect({ icon, value, onChange, options }) {
    ...
    return (
        <div ref={ref} className="relative flex-1 md:flex-none">
            <button
                ...
                className="w-full flex items-center gap-2 bg-neutral-50 border border-neutral-200 rounded-xl px-3 py-2.5 text-sm text-neutral-700 cursor-pointer hover:border-neutral-300 focus:outline-none focus:ring-2 focus:ring-emerald-600 transition-all"
            >
                <span className="text-neutral-400 shrink-0">{icon}</span>
                <span className="flex-1 text-left whitespace-nowrap">{selected?.label}</span>
                <CaretDownIcon
                    size={14}
            ...
            {open && (
                <div className="absolute z-50 mt-1.5 w-full min-w-[160px] bg-white border border-neutral-200 rounded-xl shadow-lg overflow-hidden">
                    {options.map(option => (
                        <button
                            key={option.value}
                            type="button"
...
const opcoesFormato = [
    { value: "todos", label: "Qualquer Formato" },
...
const opcoesStatus = [
    ...
    { value: "abertos", label: "Inscrições Abertas" },
