...
function statusInfo(evento) {
    ...
    if (!evento.is_publicado) return { label: "Rascunho", className: "bg-neutral-100 text-neutral-500 border border-neutral-200" };
...
function InscricaoButton({ evento }) {
    ...
    return clicked ? (
        <div className="flex items-center justify-center gap-2 rounded-2xl bg-emerald-50 border border-emerald-200 px-6 py-3.5 text-sm font-semibold text-emerald-700">
...
function AtividadeCard({ atividade }) {
    ...
    return (
        <div className={`rounded-2xl border bg-white p-5 flex flex-col gap-3 transition-shadow hover:shadow-md ${atividade.is_cancelada ? "opacity-50" : "border-neutral-200 shadow-sm"}`}>
...
export default function EventoPublico({ evento, isOwner }) { 
    
    const ev = evento;    
    const grupos = groupByDay(evento.atividades ?? []);
    const inscritosPercent = evento.limite_inscricoes
        ? Math.min(100, Math.round((evento.total_inscricoes / evento.limite_inscricoes) * 100))
        : null;
                                ...
                                {grupos.map((grupo) => (
                                    <div key={grupo.label} className="flex flex-col gap-3">
                                        ...
                                        <div className="flex flex-col gap-3">
                                            {grupo.items.map((a) => (
                                                <AtividadeCard key={a.id} activity={a} atividade={a} />
                                            ))}
                                        </div>
                                    </div>
                                ))}
