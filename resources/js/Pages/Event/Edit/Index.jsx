...
const EMPTY_FORM = {
    ...
    data_inicio: "", data_fim: "", limite_participantes: "", ministrantes: [],
};
...
function Toast({ message, onClose }) {
    ...
    return (
        <div className="fixed bottom-6 right-6 z-50 flex items-center gap-3 rounded-2xl border border-emerald-200 bg-white px-5 py-4 shadow-lg">
            ...
            <button onClick={onClose} className="ml-2 text-neutral-400 hover:text-neutral-600"><XIcon size={14} weight="bold" /></button>
        </div>
    );
}
...
function StatusBadge({ status }) {
    const map = {
        pendente:   { label: "Pendente",    className: "bg-amber-50 text-amber-700 border-amber-200",     icon: <ClockCountdownIcon size={11} weight="fill" /> },
        confirmado: { label: "Confirmado",  className: "bg-emerald-50 text-emerald-700 border-emerald-200", icon: <CheckCircleIcon size={11} weight="fill" /> },
    };
...
function AtividadeModal({ editingId, initialData, ministrantes, onClose, onSuccess }) {
    ...
    function handleNovoMinistrante() {
        if (confirm("Aviso: Você será redirecionado e os dados informados neste formulário será perdido. Deseja continuar?")) {
            router.visit(route("ministrantes.index"));
        }
    }
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center p-4">
            ...
            <div className="relative z-10 flex w-full max-w-2xl flex-col gap-6 rounded-2xl border border-neutral-200 bg-white p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
                ...
                <div className="flex flex-col gap-5">
                    <Field label="Título" value={data.titulo} onChange={(e) => setData("titulo", e.target.value)} error={errors.titulo} />
                    ...
                    <Field label="Local" value={data.local} onChange={(e) => setData("local", e.target.value)} error={errors.local} />

                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <Field label="Data de início" type="datetime-local" value={data.data_inicio} onChange={(e) => setData("data_inicio", e.target.value)} error={errors.data_inicio} />
                        <Field label="Data de fim"    type="datetime-local" value={data.data_fim}    onChange={(e) => setData("data_fim",    e.target.value)} error={errors.data_fim}    />
                    </div>

                    <Field label="Limite de participantes" type="number" min={1} value={data.limite_participantes} onChange={(e) => setData("limite_participantes", e.target.value)} error={errors.limite_participantes} />
                    ...
                    <div className="flex flex-col gap-2">
                        <div className="flex items-center justify-between">
                            ...
                            <button 
                                type="button" 
                                onClick={handleNovoMinistrante}
                                className="text-xs font-medium text-emerald-600 underline hover:text-emerald-700 transition-colors"
                            >
                                clique aqui para cadastrar um ministrante
                            </button>
                        </div>
                        ...
                        {ministrantes.length === 0 ? (
                            <div className="rounded-sm border border-dashed border-neutral-300 p-4 text-sm text-neutral-500">
                                ...
                                <button type="button" onClick={handleNovoMinistrante} className="text-emerald-600 underline">Cadastrar agora</button>
                            </div>
                        ) : (
                            <div className="flex flex-wrap gap-2">
                                {ministrantes.map((m) => {
                                    ...
                                    return (
                                        <button key={m.id} type="button" onClick={() => toggleMinistrante(m.id)}
...
export default function Index() {
    ...
    function openNew() {
        setModal({ editingId: null, initialData: { ...EMPTY_FORM, id_evento: evento.id } });
    }
    ...
    function handleDestroy(id) {
        ...
        router.delete(route("atividades.destroy", id), {
            onSuccess: () => { setToast("Atividade excluída."); router.reload({ only: ["evento"] }); },
            preserveScroll: true,
        });
    }
                    ...
                    <div className="mx-auto max-w-6xl px-6 py-8 max-md:px-4">
                        <div className="flex flex-wrap items-center justify-between gap-4">
                            <div>
                                ...
                                <p className="mt-1 text-sm text-neutral-500">Gerencie atividades, horários e ministrantes</p>
                            </div>
...
function AtividadeCard({ atividade, onEdit, onDestroy }) {
    const fmt = (date) => date ? new Date(date).toLocaleString("pt-BR", { day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit" }) : "—";
            ...
            <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="flex min-w-0 flex-col gap-3">
                    <div>
                        ...
                        {atividade.descricao && <p className="mt-1 text-sm leading-relaxed text-neutral-500">{atividade.descricao}</p>}
                    </div>
                    <div className="flex flex-wrap gap-3 text-sm text-neutral-500">
                        <span className="inline-flex items-center gap-1.5"><CalendarBlankIcon size={14} className="shrink-0" />{fmt(atividade.data_inicio)}</span>
                        <span className="text-neutral-300">→</span>
                        <span className="inline-flex items-center gap-1.5"><ClockIcon size={14} className="shrink-0" />{fmt(atividade.data_fim)}</span>
                        {atividade.local && <span className="inline-flex items-center gap-1.5"><MapPinIcon size={14} className="shrink-0" />{atividade.local}</span>}
                        {atividade.limite_participantes && <span className="inline-flex items-center gap-1.5"><UsersIcon size={14} className="shrink-0" />{atividade.limite_participantes} vagas</span>}
                    </div>

                    {/* Ministrantes vinculados diretamente */}
{atividade.ministrantes?.length > 0 && (
    <div className="flex flex-wrap gap-2">
        {atividade.ministrantes.map((m) => (
            <div key={m.id} className="flex items-center gap-1.5 rounded-full bg-neutral-50 border border-neutral-200 px-3 py-1">
                <ChalkboardTeacherIcon size={12} className="text-neutral-500" />
                ...
                <div className="flex shrink-0 items-center gap-2">
                    <button onClick={onEdit} className="inline-flex items-center gap-2 rounded-xl border border-neutral-200 px-4 py-2 text-sm font-medium text-neutral-700 hover:bg-neutral-100">
                        <PencilSimpleIcon size={15} />Editar
                    </button>
                    <button onClick={onDestroy} className="inline-flex items-center gap-2 rounded-xl border border-red-200 px-4 py-2 text-sm font-medium text-red-600 hover:bg-red-50">
                        <TrashIcon size={15} />Excluir
                    </button>
                </div>
            </div>
        </div>
