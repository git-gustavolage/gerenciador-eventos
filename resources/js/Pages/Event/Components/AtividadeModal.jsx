...
export default function AtividadeModal({ editingId, initialData, ministrantes, ambientes, onClose, onSuccess }) {
    ...
    const ambienteSelecionado = ambientes.find(a => a.id === Number(data.id_ambiente));
    const maxParticipantes = ambienteSelecionado?.capacidade || undefined;

    const ambienteOptions = ambientes.map(amb => ({
        value: amb.id,
    ...
    return (
        <>
                ...
                <div className="relative z-10 flex w-full max-w-2xl flex-col gap-6 rounded-2xl border border-neutral-200 bg-white p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
                    ...
                    <div className="flex flex-col gap-5">
                        ...
                        <div className="flex flex-col gap-1.5">
                            ...
                            <textarea value={data.descricao} onChange={(e) => setData("descricao", e.target.value)} rows={3} className="w-full rounded-sm border border-neutral-300 px-3 py-3 text-sm outline-none focus:border-emerald-500 focus:ring-emerald-300 text-neutral-700" />
                        ...
                        <div className="flex flex-col gap-5">
                            <div className="flex flex-col gap-1.5">
                                ...
                                <div className="flex gap-2">
                                    <div className="flex-1">
                                        <Select
                                            ...
                                            onChange={(selectedOption) => setData("id_ambiente", selectedOption ? selectedOption.value : "")}
                                ...
                                {errors.id_ambiente && <span className="text-xs text-red-500">{errors.id_ambiente}</span>}
                            </div>
                            ...
                            {ministrantes.length === 0 ? (
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {ministrantes.map((m) => {
                                        ...
                                        return (
                                            <button key={m.id} type="button" onClick={() => toggleMinistrante(m.id)} className={`inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-sm font-medium transition-all ${selected ? "border-emerald-500 bg-emerald-500 text-white" : "border-neutral-200 bg-white text-neutral-600 hover:border-emerald-300 hover:bg-emerald-50"}`}>
                    ...
                    <div className="flex justify-end gap-3 border-t border-neutral-100 pt-4">
                        <button type="button" onClick={onClose} className="rounded-xl border border-neutral-200 px-5 py-2.5 text-sm font-medium text-neutral-600 transition-colors hover:bg-neutral-100">Cancelar</button>
                        <PrimaryButton onClick={handleSubmit} disabled={loading}>{loading ? "Salvando..." : editingId ? "Salvar alterações" : "Criar atividade"}</PrimaryButton>
                    </div>
                </div>
            </div>
            ...
            {showAmbienteModal && (
                <AmbienteModal 
                    eventoId={data.id_evento} 
                    onClose={() => setShowAmbienteModal(false)} 
                    onSuccess={(novoAmbienteId) => {
                        if (novoAmbienteId) {
                            setData("id_ambiente", novoAmbienteId);
                        }
