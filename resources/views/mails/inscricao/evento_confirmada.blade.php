...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .btn { display: inline-block; background-color: #10b981; color: #ffffff !important; text-decoration: none; padding: 14px 32px; border-radius: 6px; font-weight: 600; font-size: 16px; }
...
<body>
  <div class="email-container">
    ...
    <div class="body">
      ...
      <p>Excelente notícia! A organização aprovou sua inscrição no evento <strong>{{ $evento->titulo }}</strong>.</p>
      <p>Agora você já pode acessar a página do evento para visualizar as palestras e oficinas disponíveis e montar a sua agenda.</p>
      <div class="btn-container">
        <a href="{{ route('participantes.evento', ['id' => $evento->id, 'tabIndex' => 1]) }}" class="btn">Ver Programação</a>
      </div>
      ...
      <div class="link-box" style="background-color: #f9fafb; padding: 16px; border-radius: 6px; text-align: center; border: 1px dashed #d1d5db; word-break: break-all;">
        ...
        <p class="small" style="margin: 0;"><a href="{{ route('participantes.evento', ['id' => $evento->id, 'tabIndex' => 1]) }}" style="color: #10b981; font-size: 13px; text-decoration: underline;">{{ route('participantes.evento', ['id' => $evento->id, 'tabIndex' => 1]) }}</a></p>
      </div>
    </div>
    <div class="footer">
